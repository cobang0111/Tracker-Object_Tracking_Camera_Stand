...
tracker = DeepSort(max_age=50) #DeepSort 정의
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    #if label == 0:
    results = [[xmin, ymin, xmax-xmin, ymax-ymin]]        

    # DeepSort Algorithm
    tracks = tracker.update_tracks(results, frame=img)

    if not tracks[0].is_confirmed():
        return
    ...
    cv2.putText(img, str(track_id), (xmin + 5, ymin - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, WHITE, 2)
...
class YoLov7TRT(object):
    ...
    def post_process(self, output, origin_h, origin_w):
        ...
        boxes = self.non_max_suppression(
            pred, origin_h, origin_w, conf_thres=CONFIDENCE_THRESHOLD, nms_thres=IOU_THRESHOLD
        )
    ...
    def non_max_suppression(
        ...
        """
        # Get the boxes that score > CONFIDENCE_THRESHOLD

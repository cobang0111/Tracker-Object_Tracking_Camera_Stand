...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    #result = [[[int(x[0]), int(x[1]), int(x[2]), int(x[3])]]]
    
    #track = tracker.update_tracks(result, frame=img)

    #if not track[0].is_confirmed():
        #return
    
    #track_id = track[0].track_id
    #ltrb = track[0].to_ltrb()

    #xmin, ymin, xmax, ymax = int(ltrb[0]), int(ltrb[1]), int(ltrb[2]), int(ltrb[3])

    #cv2.rectangle(img, (xmin, ymin), (xmax, ymax), color, thickness=tl, lineType=cv2.LINE_AA)
    # Draw id rectangle
    #cv2.rectangle(img, (xmin, ymin - 20), (xmin + 20, ymin), color, -1, cv2.LINE_AA)  # filled
    # Input id in rectangle
    #cv2.putText(img, str(track_id), (xmin + 5, ymin - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    ...
    def run(self):
        ...
        while True:
            ...
            if abs(dx) >= 64:
                motor1_control(dx)
                frame_count = 0
                print("x_control {:.2f}".format(duty_cycle))
            #if dy >= 10:
                #motor2_control(y, dy, use_time)
                #print("y_control")
            cv2.putText(
                result,
                fps,
